...
<head>
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Content is visible by default; the hidden initial state only applies
       when JS is active (html.js), so disabled/blocked JS never hides content. */
    .reveal { opacity: 1; transform: none; }
    .js .reveal { opacity: 0; transform: translateY(30px); transition: all 0.8s cubic-bezier(0.4, 0, 0.2, 1); }
...
<body>
  ...
  <script>
    ...
    // Scroll reveal (with safe fallbacks so content is never stuck hidden)
    ...
    if ('IntersectionObserver' in window) {
      ...
      // Failsafe: if anything is still hidden after 2.5s, just show it.
      setTimeout(() => {
